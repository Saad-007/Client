    ...
    <div className="bg-stone-50 min-h-screen antialiased">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 md:py-16">
        ...
        <section className="mb-16 md:mb-24 text-center">
          ...
          >
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-stone-900 mb-4 md:mb-6">AI Resume Analysis</h2>
            <p className="text-base sm:text-lg md:text-xl text-stone-600 mb-6 md:mb-10 leading-relaxed">
            ...
            <motion.div
              ...
              whileTap={{ scale: 0.98 }}
              className="border-2 border-dashed border-stone-300 rounded-xl md:rounded-2xl p-6 md:p-12 cursor-pointer hover:border-amber-400 transition-colors bg-white"
              onDrop={handleDrop}
              onDragOver={handleDragOver}
            >
              <div className="flex flex-col items-center">
                <FiUpload className="text-3xl md:text-4xl text-amber-600 mb-3 md:mb-4" />
                {file ? (
                  <>
                    <p className="font-medium text-stone-800 text-base md:text-lg mb-2 truncate max-w-xs">{file.name}</p>
                    <p className="text-stone-500 text-sm">{(file.size / 1024 / 1024).toFixed(2)} MB</p>
                    <button 
                      onClick={() => setFile(null)}
                      className="mt-4 md:mt-6 bg-stone-100 hover:bg-stone-200 text-stone-800 px-4 py-2 md:px-6 md:py-3 rounded-lg font-medium transition-colors text-sm md:text-base"
                    >
                      Change File
                    </button>
                  </>
                  ...
                  <>
                    <p className="font-medium text-stone-800 text-base md:text-lg mb-2">Drag & Drop Your Resume Here</p>
                    <p className="text-stone-500 text-sm">PDF, DOCX, or TXT (Max 5MB)</p>
                    <label className="mt-4 md:mt-6 bg-stone-100 hover:bg-stone-200 text-stone-800 px-4 py-2 md:px-6 md:py-3 rounded-lg font-medium transition-colors cursor-pointer text-sm md:text-base">
                      Select File
                      <input 
                        type="file" 
                        className="hidden" 
            ...
            <div className="mt-6 md:mt-8 max-w-2xl mx-auto">
              <label htmlFor="jobDescription" className="block text-left text-stone-700 mb-2 text-sm md:text-base">
                Optional: Paste job description for targeted analysis
              </label>
              <textarea
                id="jobDescription"
                rows={3}
                className="w-full px-3 py-2 md:px-4 md:py-3 border border-stone-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm md:text-base"
            ...
            <button
              ...
              className={`mt-6 md:mt-8 px-6 py-3 md:px-8 md:py-4 rounded-lg md:rounded-xl font-bold text-base md:text-lg ${file && !isLoading ? 'bg-gradient-to-r from-amber-500 to-amber-600 hover:shadow-lg' : 'bg-stone-300 cursor-not-allowed'} transition-all`}
            >
              {isLoading ? 'Analyzing...' : 'Analyze My Resume'}
            </button>
            
            <div className="mt-6 md:mt-8 flex flex-wrap justify-center gap-2 md:gap-4">
              <div className="flex items-center bg-stone-100 rounded-full px-3 py-1 md:px-4 md:py-2 text-xs md:text-sm">
                <FiCheck className="text-amber-600 mr-1 md:mr-2" />
        ...
        {analysis.overallScore > 0 && (
          <section id="analysis-results" className="bg-white rounded-xl md:rounded-3xl shadow-sm border border-stone-200 overflow-hidden mb-16 md:mb-24">
            {/* Results Header */}
            <div className="bg-stone-900 text-white p-4 md:p-8">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-6">
                <div>
                  <h3 className="text-xl md:text-2xl font-medium mb-2">Analysis Results</h3>
                  <p className="text-stone-300 text-sm md:text-base">Generated on {new Date().toLocaleDateString()}</p>
                </div>
                <div className="flex items-center gap-2 md:gap-4">
                  <button 
                    onClick={downloadReport}
                    className="flex items-center gap-1 md:gap-2 bg-white hover:bg-stone-100 text-stone-800 px-3 py-2 md:px-6 md:py-3 rounded-lg font-medium transition-colors border border-stone-300 text-sm md:text-base"
                  >
            ...
            {/* Score Overview */}
            <div className="p-4 md:p-8 border-b border-stone-200">
              <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8">
                <div className="relative w-32 h-32 md:w-40 md:h-40">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-3xl md:text-4xl font-bold text-stone-800">{analysis.overallScore}</span>
                    <span className="text-stone-500 text-sm">out of 100</span>
                  </div>
                </div>
                
                <div className="flex-1">
                  <h4 className="text-lg md:text-xl font-medium text-stone-800 mb-3 md:mb-4">Overall Resume Score</h4>
                  ...
                  <div className="w-full bg-stone-100 rounded-full h-2 md:h-3 mb-1 md:mb-2">
                    <div 
                      className="bg-gradient-to-r from-amber-400 to-amber-600 h-2 md:h-3 rounded-full" 
                      style={{ width: `${analysis.overallScore}%` }}
                    ></div>
                  </div>
                  <div className="flex justify-between text-xs md:text-sm text-stone-500">
            ...
            {analysis.jobTitleMatch && (
              <div className="p-4 md:p-8 border-b border-stone-200">
                <h4 className="text-lg md:text-xl font-medium text-stone-800 mb-3 md:mb-4">Job Match Analysis</h4>
                <div className="bg-blue-50 border border-blue-200 rounded-lg md:rounded-xl p-4 md:p-6 mb-4 md:mb-6">
                  <p className="text-blue-800 font-medium text-sm md:text-base">{analysis.jobTitleMatch}</p>
                </div>
                
                {analysis.keywordMatches.length > 0 && (
                  <>
                    <h5 className="font-medium text-stone-700 mb-2 md:mb-3 text-sm md:text-base">Keyword Matches</h5>
                    <div className="flex flex-wrap gap-2 md:gap-3 mb-3 md:mb-4">
                      {analysis.keywordMatches.map((keyword, index) => (
                        <span 
                          key={index}
                          className={`px-2 py-1 md:px-3 md:py-1 rounded-full text-xs ${
            ...
            {/* Category Breakdown */}
            <div className="p-4 md:p-8 border-b border-stone-200">
              <h4 className="text-lg md:text-xl font-medium text-stone-800 mb-4 md:mb-6">Detailed Breakdown</h4>
              ...
              <div className="grid md:grid-cols-2 gap-4 md:gap-6 mb-8 md:mb-10">
                {analysis.categories.map((category, index) => (
                  <motion.div
                    key={index}
                    whileHover={{ y: -3 }}
                    className="bg-stone-50 rounded-lg md:rounded-xl p-4 md:p-6 border border-stone-200"
                  >
                    <div className="flex justify-between items-start mb-3 md:mb-4">
                      <h5 className="font-medium text-stone-800 text-sm md:text-base">{category.name}</h5>
                      <span className={`px-2 py-1 md:px-3 md:py-1 rounded-full text-xs md:text-sm font-medium ${getScoreColor(category.score*10)}`}>
                        {category.score}/10
                      </span>
                    </div>
                    <p className="text-stone-600 text-xs md:text-sm">{category.feedback}</p>
                  </motion.div>
                ))}
              </div>
            </div>
            ...
            <div className="p-4 md:p-8">
              <h4 className="text-lg md:text-xl font-medium text-stone-800 mb-4 md:mb-6">Actionable Suggestions</h4>
              <ul className="space-y-3 md:space-y-4">
                {analysis.suggestions.map((suggestion, index) => (
                  <li key={index} className="flex items-start gap-2 md:gap-3">
                    <div className={`mt-1 flex-shrink-0 w-4 h-4 md:w-5 md:h-5 rounded-full flex items-center justify-center ${
                      index % 2 === 0 ? "bg-amber-100 text-amber-600" : "bg-stone-100 text-stone-600"
                    }`}>
                      <FiChevronRight className="text-xs" />
                    </div>
                    <p className="text-stone-700 text-sm md:text-base">{suggestion}</p>
                  </li>
                ))}
              </ul>
            </div>
    ...
    <div className="text-center mt-6">
      <button className="inline-flex items-center text-sm text-amber-600 hover:text-amber-700 font-medium">
        ...
        <section className="bg-gradient-to-r from-stone-900 to-stone-800 rounded-xl md:rounded-3xl p-6 md:p-12 text-center text-white mb-16 md:mb-24">
          ...
          <p className="text-stone-300 max-w-2xl mx-auto mb-6 md:mb-8 text-sm md:text-lg">
          ...
          <button className="bg-gradient-to-r from-amber-500 to-amber-600 text-stone-900 px-6 py-3 md:px-8 md:py-4 rounded-lg md:rounded-xl font-bold text-base md:text-lg hover:shadow-lg transition-all">
        ...
        <section className="max-w-4xl mx-auto py-8 md:py-12 px-4">
          <h3 className="text-xl md:text-2xl font-bold text-stone-900 mb-6 md:mb-8 text-center">Frequently Asked Questions</h3>
          
          <div className="space-y-3 md:space-y-4">
            {faqData.map((item, index) => (
              <div key={index} className="border border-stone-200 rounded-lg md:rounded-xl overflow-hidden transition-all duration-300">
                <button 
                  className="w-full flex justify-between items-center p-4 md:p-6 text-left hover:bg-stone-50 transition-colors"
                  onClick={() => toggleFAQ(index)}
                  aria-expanded={activeIndex === index}
                >
                  <span className="font-medium text-stone-800 text-sm md:text-base">{item.question}</span>
                  {activeIndex === index ? (
                    <FiChevronUp className="text-stone-500 transition-transform flex-shrink-0" />
                  ) : (
                    <FiChevronDown className="text-stone-500 transition-transform flex-shrink-0" />
                  )}
                </button>
