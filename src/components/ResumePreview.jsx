...
const ResumePreview = forwardRef(({ resume, template = 'professional', templateStyles }, ref) => {
  ...
  if (typeof resume === 'string') {
    return (
      <div 
        ref={ref} 
        className="p-4 md:p-6 rounded-xl md:rounded-2xl shadow-lg border border-gray-200 print:p-6 print:rounded-2xl"
        style={{
          ...style,
          backgroundColor: 'var(--bg-color)',
          width: '100%',
          maxWidth: '210mm', // For mobile, use 100% width but limit to A4 max width
        }}
      >
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:justify-between md:items-start">
                      ...
                      {year && (
                        <span className="text-xs md:text-xs mt-1 md:mt-0 md:ml-2" style={{ color: 'var(--text-color)' }}>{year}</span>
                      )}
                    </div>
                  </li>
                );
